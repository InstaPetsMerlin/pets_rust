...
impl User {
    ...
    pub fn get_user_by_username(username: &String, conn: &PgConnection) -> Result<Vec<User>, Error> {
        return match all_users.filter(users::username.eq(username)).load::<User>(conn) {
            Ok(users) => Ok(users),
            Err(e) => Err(e),
        };
    }
